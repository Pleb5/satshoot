...
- Bitcoiners pay for troubleshooting based on an agreed fee. The fee can be time or milestone based
- If an offer is accepted, troubleshooting can begin via any means of communication, defaulting to nostr DM-s

- Tech stack: Nostr Devkit PWA; Nostr connect; Nostr wallet connect; LN zaps; App-specific event kinds
...
# Bitcoin Troubleshoot
The "decentralized customer support" of bitcoin.
...
**A P2P marketplace for solving problems related to bitcoin self-custody, using bitcoin and nostr: Bitcoin Troubleshoot app, the decentralized customer support service of bitcoin**
...
- UX:
    ...
    - We need to protect against bad hehavior to facilitate trust between Bitcoiners and Troubleshooters
...
1. Bitcoiner creates a guided issue description with title, body and tags like "Wallet Software", "Sparrow", "Bluewallet", "Node", "Raspiblitz", "RoninDojo", "Tor", "VPN", "Hardware wallet", "Seedsigner", "Coldcard", etc.(like on forums)
    1. He can also create milestones for the ticket. E.g. Download, install and verify Sparrow wallet --> Connect to my node --> Learn the basics of Sparrow
...
4. The Troubleshooter makes a proposal to solve the ticket:
    1. Sats/minute, Sats/milestone or some combination. Fees are billed manually by the Troubleshooter and paid manually by the Bitcoiner. Automatic payments are too error-prone and would need a lot of heavy-lifting to be implemented safely. The assumption is that the Troubleshooter will watch out for himself not to work for free, while the Bitcoiner is never charged accidentally
...
8. The Troubleshooter who won the job is alerted and has to start resolving the issue
    ...
    4. The Bitcoiner may reopen the ticket as new after giving up on that Troubleshooter
...
# Architecture
The "orderbook" lives on nostr. Bitcoiners and Troubleshooters run the same nostr client which only listens to the necessary events regarding the service.

## Nostr
Discovery, contract negotiation and execution is solved with nostr. Communication can be solved with nostr or other tools. The best would be to define or extend a NIP for this of use-case but right now it is enough to just start with app-specific event kinds and create a **dedicated open-source client** for the best user-experience.
...
- The contract negotiation and the Troubleshooting itself will make use of the NIP4 encrypted DMs if not negotiated otherwise(nostr could be just the contracting layer and comms could be taken to e.g. SimpleX)
...
### Ticket states
On nostr a ticket state is just a parameterized replaceable event
- "New": Newly posted ticket with no proposals accepted yet
- "Taken": The Bitcoiner has taken one of the proposals of Troubleshooters
- The state can go back to "New" if the troubleshooting process fails
- Only "New" tickets are of interest to Troubleshooters

## Payments
LN Zaps. Base-layer transactions are too slow and lack privacy. Since amounts should not be too high per job or per milestone, LN is much better for this purpose.

Payment failure has to be displayed in a conspicuous way for the Troubleshooter (and the Bitcoiner too) whenever it happens.

There could be features developed in the future that help facilitate contract execution between parties e.g. a time logger that is controlled by the Troubleshooter and can be monitored by the Bitcoiner.
